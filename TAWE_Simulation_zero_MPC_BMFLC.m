%% The Wearable Wrist Exoskeleton (TAWE) Control Simulation with tremor suppression (Figs 8)
...
% BMFLC Model for tremor modeling
...
for ii=1:numel(tSpan1)
    ...
    %BMFLC Jacobian Matrix
    tr_jac = inv(JacU2Tpose)*JacU1Tpose*BMFLCJac.';

    %Obtain optimal input from MPC Controller
    taweInput = mpc_with_BMFLC(Ad,Bd,P,Q,R,S_gain,hor,X_in,Consterm,path2, uMax, uMin, xx_lb, xx_ub, tr_jac, hh);
...
save(strcat('./Data/zero_track_bmflc'),'simStateData') % Save result
save(strcat('./Data/zero_track_bmflc_inp'),'simInputData')
